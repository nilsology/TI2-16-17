 ...
 */
void benchmark(uint64_t (*func)(uint64_t), uint64_t val, Measurement* m) {
...
int main(int argc, char* argv[]) {
	...
	// C
	benchmark(c_fib_it, val, &m_ergs_it);
	benchmark(c_fib_rek, val, &m_ergs_rek);
	drawTable("C", &m_ergs_it, &m_ergs_rek);
	
	// ASM
	benchmark(asm_fib_it, val, &m_ergs_it);
	benchmark(asm_fib_rek, val, &m_ergs_rek);
